...
```{r}
df_red <- read.csv("winequality-red.csv",
               sep = ';')
df_red
```
...
df_red <- rename_fun(df_red)
df_red
```

```{r}
df_red %>% summarise(na_count = sum(is.na(df_red)))
...
```{r}
red_fit <- lm(density ~ fixed_acidity, data = df_red)
...
ggplot(df_red)+
  geom_smooth(aes(x= density, y=fixed_acidity))+
  geom_point(aes(x= density, y=fixed_acidity, color = pH))
